...
const ProductGallery = ({modalState, setModalState} : IProductGalleryProps) => {

    return (
        <section className='product-gallery'>
            <div className="product-gallery__title">
            ...
            <div className="product-gallery__wrapper">
                {photoList.map((photo, i) => {
                    i = i + 1;
                    return (
                        <div key= {i} className={'product-gallery__item item-' + i} onClick={() => setModalState(i - 1)}>
                            <img src={photo} alt="" />
                        </div>
                    )
                })}
            ...
            {modalState !== null
                ?
                <div className="product-gaallery__module"
                    onClick={(e) => setModalState(null)}
                >
                    <div className="product-gallery__module_image" onClick={e => e.stopPropagation()}>
                        <img src={photoList[modalState]} alt="" />
                    </div>
                    <div className="product-gallery__module_navigation" onClick={e => e.stopPropagation()}>
                        <div className="prev"
                            onClick={() => {
                                if (modalState !== 0) {
